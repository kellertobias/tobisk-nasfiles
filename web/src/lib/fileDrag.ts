...
function buildDragPreview(payload: FileDragPayload, preview: FileDragPreview = {}) {
...
export function setFileDragPayload(dataTransfer: DataTransfer, payload: FileDragPayload, preview?: FileDragPreview) {
...
export function getFileDragPayload(dataTransfer: DataTransfer): FileDragPayload | null {
  const raw = dataTransfer.getData(NASFILES_DRAG_TYPE) || dataTransfer.getData('text/plain');
  if (!raw) return null;
...
export function getDemoDraggingPayload() {
  const parsed = readJsonLocalStorage<Partial<FileDragPayload>>(NASFILES_DEMO_DRAGGING_KEY);
  if (
    parsed &&
    typeof parsed.root === 'string' &&
    Array.isArray(parsed.paths) &&
...
export function getDemoDropTarget() {
  const parsed = readJsonLocalStorage<Partial<FileDropTarget>>(NASFILES_DEMO_DROP_TARGET_KEY);
  if (parsed && typeof parsed.root === 'string' && typeof parsed.path === 'string') {
...
export function isDemoDraggedPath(root: string, path: string) {
  ...
  return Boolean(payload && payload.root === root && payload.paths.includes(path));
}

export function getDemoDragHoverPayload() {
  const parsed = readJsonLocalStorage<Partial<FileDragPayload>>(NASFILES_DEMO_DRAG_HOVER_KEY);
  if (
    parsed &&
    typeof parsed.root === 'string' &&
    Array.isArray(parsed.paths) &&
...
export function isSelfOrDescendantDrop(payload: FileDragPayload, targetRoot: string, targetPath: string) {
  if (payload.root !== targetRoot) return false;
  return payload.paths.some((path) => targetPath === path || targetPath.startsWith(`${path}/`));
}
