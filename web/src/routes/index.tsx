...
function IndexPage() {
  ...
  useEffect(() => {
    if (user && user.roots.length > 0) {
      ...
      const defaultRoot = user.roots.find((r) => r.kind === 'home') || user.roots[0];
      navigate({
        to: '/r/$root/$',
    ...
    return (
      ...
      {mode === 'sso' ? (
        ...
        <form
          ...
          style={{ display: 'grid', gap: 'var(--space-3)', width: 'min(360px, calc(100vw - 32px))' }}
        >
          <input
            value={username}
          ...
          {totpChallenge && (
            <>
              <input
                ...
                style={inputStyle}
              />
              <label style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', fontSize: 'var(--text-sm)', color: 'var(--color-fg-muted)' }}>
                <input type="checkbox" checked={trustComputer} onChange={(e) => setTrustComputer(e.target.checked)} />
                Trust this computer
              </label>
            </>
          )}
          <button type="submit" disabled={!username || (!totpChallenge && !password) || loginMutation.isPending || totpMutation.isPending} style={buttonStyle}>
          ...
          {totpChallenge && (
            <button type="button" onClick={() => { setTotpChallenge(null); setTotpCode(''); }} style={secondaryButtonStyle}>
              Back
            </button>
          )}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        No folders are configured yet. Set the <code style={{
          padding: '1px 4px',
          background: 'var(--color-bg-muted)',
          borderRadius: 'var(--radius-sm)',
          fontSize: 'var(--text-xs)',
        }}>COMMON_FOLDERS</code> environment variable to get started.
      </div>
    </div>
  );
}
